...
NativeExecResult NativeCatalog::exec_insert(std::string_view sql, const std::vector<std::string>& params) {
  ...
  std::string table = trim(rest.substr(0, paren));
  auto vals_kw = lower(rest).find(" values ");
  if (vals_kw == std::string::npos) return result;
  auto cols_part = trim(rest.substr(paren + 1, rest.find(')') - paren - 1));
  std::string vals_part = trim(rest.substr(rest.find("values") + 6));
  if (!vals_part.empty() && vals_part.front() == '(') vals_part.erase(0, 1);
  if (!vals_part.empty() && vals_part.back() == ')') vals_part.pop_back();
  ...
  if (!persist_row(table, row)) return result;
